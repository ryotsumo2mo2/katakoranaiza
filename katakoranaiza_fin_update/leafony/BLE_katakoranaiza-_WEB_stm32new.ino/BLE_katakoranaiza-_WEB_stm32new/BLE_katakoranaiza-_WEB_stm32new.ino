...
//------------------------------
#define LOOP_INTERVAL 5000                // 125000us = 125ms interval 変更点
...
void setupSensor(){
  ...
  accel.setRange(LIS3DH_RANGE_4_G);          // Full scale +/- 2G 変更点
  accel.setDataRate(LIS3DH_DATARATE_100_HZ);  // Data rate = 10Hz 変更点
...
void loop() {
  ...
  if (bInterval == true){
    ...
    mil_a=millis(); //追加点
    Serial.println((mil_a-mil_b)); //追加点
    mil_b=mil_a; //追加点
...
//--------------------------------------------以下追加点
    ...
    if(nn==0){
      ...
      Serial.println("計測開始");
      sakiire[0] = 9.99;
  for(ni=1;ni<de;ni=ni+3){
  accel.read();
  sakiire[ni] = accel.x_g;
  sakiire[ni] = accel.y_g;
  sakiire[ni] = accel.z_g;
  Serial.println((sakiire[ni]));
  //delay(1);
  }
      Serial.println("配列完了");
      //delay(5000);
...
void bt_sendData(){
  float value;
  char Xg1[7], Xg2[7], Xg3[7], Xg4[7],Xg5[7], Xg[7]; //変更点
  //char code[4]; //追加点
  char sendData[40];
  uint8 sendLen;
  ...
  //-------------------------
  //-------------------------
  // Temperature (4Byte)変更点・加速度センサーデータ取得始まり
  //-------------------------
  value = sakiire[nd];

  dtostrf(value,4,2,Xg1);
nd = nd + 1;
if(nd==de){
  nd = 0;
  }
...
//--------------------------------------変更点・加速度センサーデータ取得終わり
  ...
  if (dispLCD==1){
    switch (lcdSendCount){
      ...
      case 1:                 // Tmp XX.X [degC]
        lcd.print("Temp");
        lcd.setCursor(0, 1);
        lcd.print( String(Xg1) +" C");
        break;
      case 2:                 // Hum xx.x [%]
        lcd.print("Humidity");
        lcd.setCursor(0, 1);
        lcd.print( String(Xg2) +" %");
        break;
      case 3:                 // Lum XXXXX [lx]
        lcd.print("Luminous");
        lcd.setCursor(0, 1);
        lcd.print( String(Xg3) +" lx");
        break;
      case 4:                 // Ang XXXX [arc deg]
        lcd.print("Angle");
        lcd.setCursor(0, 1);
        lcd.print( String(Xg4) +" deg");
        break;
      case 5:                 // Bat X.XX [V]
        lcd.print("Battery");
        lcd.setCursor(0, 1);
        lcd.print( String(Xg) +" V");
        break;
      default:
        break;
    }
  ...
  if( bBLEsendData == true ){                                 // BLE transmission
    ...
    sendLen = sprintf(sendData, "%04s,%04s,%04s,%04s,%04s,%04s\n", Xg1, Xg2, Xg3, Xg4, Xg5, Xg); //変更点
    // Send to BLE device
...
// To display on a single line
  Serial.println("###\tbt_sendDatasend: { Temp=" + String(Xg1) + ", Humid=" + String(Xg2) + ", Light=" + String(Xg3) + ", Tilt=" + String(Xg4) + ", Vbat=" + String(Xg5) + ", Dice=" + String(Xg) + " }"); //変更点
Serial.println(nd);
#endif
}
//====================================================================
